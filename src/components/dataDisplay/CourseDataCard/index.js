...
function CourseDataCard({ course }) {
    ...
    return (
        <Card title={translate(TEXTS.COURSE_DATA_CARD_TITLE)} >
            ...
            {course?.accessPin && <>
                <LabelValue label={translate(TEXTS.COURSE_ACCESS_PIN_LABEL)} value={course?.accessPin}></LabelValue>
                <LabelValue label={translate(TEXTS.COURSE_URL_LABEL)} value={<Link href={url}>{url}</Link>} valueProps={{ sx: { overflowWrap: 'anywhere', } }}></LabelValue>
            </>}
        </Card >
    )
}
